...
test('coordinate validation accepts longitude boundaries without wrapping', () => {
  ...
  for (const longitude of [-180.0000001, 180.0000001]) {
    assert.deepEqual(validateCoordinate(longitude, 0), {
      status: 'invalid',
      reason: 'longitude',
    })
  }
...
test('coordinate validation distinguishes geographic and Web Mercator limits', () => {
  for (const latitude of [
    ...
    assert.equal(result.status, 'projectable')
    if (result.status === 'projectable')
      assert.equal(result.projectionClamped, false)
  }
...
test('coordinate validation rejects non-finite values', () => {
  for (const value of [Number.NaN, Infinity, -Infinity]) {
    assert.deepEqual(validateCoordinate(value, 0), {
      status: 'invalid',
      reason: 'non-finite',
    })
    assert.deepEqual(validateCoordinate(0, value), {
      status: 'invalid',
      reason: 'non-finite',
    })
  }
...
test('initial parsing and recoloring share coordinate acceptance', () => {
  ...
  const acceptedForParsing = coordinates.map(
    ([lon, lat]) => validateCoordinate(lon, lat).status === 'projectable'
  )
  const acceptedForRecoloring = coordinates.map(
    ([lon, lat]) => validateCoordinate(lon, lat).status === 'projectable'
  )
