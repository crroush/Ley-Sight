...
export function ViewshedApp() {
  ...
  const [resultState, setResultState] = useState<ViewshedResultState>(
    initialViewshedResultState
  )
    ...
    workerRef.current.onmessage = (event) => {
      try {
        if (!event.data || typeof event.data !== 'object')
          throw new Error('The worker returned an empty or malformed response.')
        ...
        if (!Number.isInteger(payloadRunId)) {
          updateResult({
            ...
            timestamp: Date.now(),
            message:
              'The worker response did not include a valid run identifier.',
          })
        ...
        if (event.data.type === 'COMPUTE_COMPLETE') {
          ...
          if (!payloadError) {
                ...
                if (runId === runIdRef.current) {
                  ...
                  if (blob && mapRef.current) {
                    ...
                    updateResult({
                      ...
                      runId,
                      timestamp: Date.now(),
                      message: 'The viewshed image could not be serialized.',
                    })
                  } else {
                    updateResult({
                      type: 'ERROR',
                      ...
                      timestamp: Date.now(),
                      message:
                        'The map is unavailable for the viewshed result.',
                    })
                  }
                }
              }, 'image/png')
            ...
            updateResult({
              ...
              timestamp: Date.now(),
              message:
                'The browser could not create a canvas for the viewshed image.',
            })
          } else {
            updateResult({
              type: 'ERROR',
          ...
          const message =
            typeof payload.error === 'string' && payload.error.trim()
              ? payload.error
              : 'The viewshed worker failed without an error message.'
  ...
  const invalidateAndRecompute = useCallback(() => {
    ...
    debounceTimerRef.current = window.setTimeout(() => triggerCompute(true), 50)
  }, [triggerCompute])
  ...
  const fetchDemProfile = async (
    ...
    const finalTgtAlt = elevations[samples] + targetHeightRef.current * 1000
  ...
  useEffect(() => {
    ...
    const handleMapChange = () => {
      if (debounceTimerRef.current !== null)
        window.clearTimeout(debounceTimerRef.current)
      debounceTimerRef.current = window.setTimeout(() => triggerCompute(), 400)
    }
  ...
  const pendingObserverInputStyle = (idx: number, field: keyof Observer) => {
    const pending =
  ...
  const renderActiveTabContent = () => {
    ...
    if (activeRes.error) {
      return (
        <div
          role="alert"
          style={{ padding: '24px', color: '#991b1b', fontWeight: 600 }}
        >
            ...
            >
              <thead>
                <tr style={{ background: '#f1f5f9', color: '#475569' }}>
                  ...
                  <th style={{ padding: '8px', width: '280px' }}>
                    Sensor height
                  </th>
                      ...
                      {obs.kind === 'ground' ? (
                        ...
                        >
                          <input
                            ...
                            step="1"
                            value={
                              antennaHeightDrafts[idx] ??
                              String(obs.antennaHeightAglM)
                            }
                    ...
                    >
                      {obs.kind !== 'geo' && (
                        <button
                          ...
                          style={{
                            background:
                              mapPickWaitingIdx === idx ? '#fef08a' : '#e2e8f0',
                            border: '1px solid #94a3b8',
                            padding: '2px 6px',
                            cursor: 'pointer',
                            borderRadius: '4px',
