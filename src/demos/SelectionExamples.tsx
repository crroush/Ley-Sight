...
export function FeatureSelectionExampleApp() {
  ...
  useEffect(() => {
    ...
    const layers = sources.map((source) =>
      new VectorLayer({source, style: normalStyle})
    );
    const map = new Map({
    ...
    const updateLabel = (): void => {
      ...
      setSelectionText(
        `Selected ${total} feature(s): ${
          Array.from(breakdown, ([layer, count]) =>
            `${layer.split("_")[0]}: ${count}`
          ).join(", ")
        }`,
      );
    };
    ...
    <main className="reference-example-window">
      <section className="reference-instruction-panel">
        <p>
          ...
          • Ctrl/Cmd+Click to add to selection (multi-select)<br />
          • Ctrl/Cmd+Drag to box select multiple points<br />
...
function recolorSample() {
  ...
  const track = new Feature(new GeometryCollection(
    expandedTrackCoordinates.slice(1).map((coordinate, index) =>
      new LineString([
        fromLonLat([
...
function recolorStyle(feature: FeatureLike): Style | Style[] {
  ...
  return geometry.getGeometries().map((segment, index) =>
    new Style({
      geometry: segment,
      stroke: new Stroke({
        color: override
...
export function SelectionRecolorExampleApp() {
  ...
  useEffect(() => {
    ...
    const layers = sources.map((source) =>
      new VectorLayer({source, style: recolorStyle})
    );
