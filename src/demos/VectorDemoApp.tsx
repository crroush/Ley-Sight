...
function demoFeatures(): Feature[] {
  ...
  const ellipseFeature = new Feature(
    ellipse(fromLonLat([-87.5, 41.9]) as [number, number], 320_000, 120_000)
  );
  ...
  for (let index = 1; index < track.length; index += 1) {
    const segment = new Feature(
      new LineString([fromLonLat(track[index - 1]), fromLonLat(track[index])])
    );
...
export function VectorDemoApp() {
  ...
  useEffect(() => {
    ...
    const map = new Map({
      ...
      layers: [baseLayer, countryLayer, hydrologyLayer, vectorLayer],
      view: new View({
      ...
      <main className="demo-content">
        <div className="demo-map">
          ...
          <div className="demo-overlay-note">
            Click to select · drag selected features to move · gradient track is
            fixed
          </div>
        </div>
        <aside className="demo-sidebar">
          <section className="demo-status-card">
            ...
            <p>
              Select any movable feature, then drag it. Enable Modify vertices
              for line and polygon handles.
            </p>
          </section>
          <section>
            <h2>
              <PaintBucket size={14} /> Recolor selection
            </h2>
