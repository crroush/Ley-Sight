...
function IntegrationTable({
  ...
  const [sort, setSort] = useState<{
    column: keyof Pick<IntegratedRow, "layer" | "type" | "id" | "value">;
    descending: boolean;
  }>({column: "layer", descending: false});
  const displayRows = useMemo(() => {
    const output = rows.map((_, index) => index);
    output.sort((first, second) => {
      const comparison = rows[first][sort.column].localeCompare(
        rows[second][sort.column],
        undefined,
        {numeric: true},
      );
  ...
  return (
    <section className="qt-table-frame">
      <div className="qt-table-header qt-integration-columns">
      ...
      <div className="qt-table-scroll" ref={scrollRef}>
        ...
        >
          {virtualizer.getVirtualItems().map((item) => {
            const row = rows[displayRows[item.index]];
            return (
              <button
                type="button"
                className={`qt-table-row qt-integration-columns ${
                  selected.has(row.key) ? "is-selected" : ""
                }`}
                key={row.key}
                style={{transform: `translateY(${item.start}px)`}}
                onContextMenu={(event) => {
                  event.preventDefault();
                  onContextMenu(event.clientX, event.clientY, row.key);
                }}
...
export function TableIntegrationExampleApp() {
  ...
  const randomRef = useRef(createQtRandom(42));
  ...
  return (
    <div className="qt-example-window qt-integration-window">
      ...
      <main
        className="qt-integration-layout"
        ref={layoutRef}
        style={{
          gridTemplateColumns:
      ...
      >
        <aside className="qt-integration-controls">
          ...
          <button type="button" className="qt-delete-button" onClick={deleteSelected}>
      ...
      {menu && (
        <div
          className="qt-context-menu qt-table-context-menu"
