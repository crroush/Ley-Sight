...
function VirtualGrid({
  ...
  const sortValuesRef = useRef(sortValues);
  sortValuesRef.current = sortValues;
  const [sort, setSort] = useState<{column: number; descending: boolean} | null>(
    null,
  );
  const order = useMemo(() => {
    if (!sort) return null;
    const valueFor = sortValuesRef.current[sort.column];
    const output = Array.from({length: count}, (_, row) => row);
    output.sort((first, second) => {
      ...
      const stable = difference || first - second;
      return sort.descending ? -stable : stable;
    });
    return output;
  }, [count, sort]);
  ...
  useEffect(() => {
    if (firstSelected != null && firstSelected < count) {
      const position = order ? order.indexOf(firstSelected) : firstSelected;
      if (position >= 0) virtualizer.scrollToIndex(position, {align: "auto"});
    }
  }, [count, firstSelected, order, virtualizer]);

  return (
    <section className="qt-table-frame">
      <div className="qt-table-header" style={{gridTemplateColumns: columns}}>
      ...
      <div className="qt-table-scroll" ref={scrollRef}>
        ...
        >
          {virtualizer.getVirtualItems().map((item) => {
            const row = order?.[item.index] ?? item.index;
            return (
            <button
              type="button"
              className={`qt-table-row ${selected.has(row) ? "is-selected" : ""}`}
              key={rowKey(row)}
              style={{
              ...
              onClick={(event: ReactMouseEvent<HTMLButtonElement>) => {
                ...
                if (event.shiftKey && anchorRef.current != null) {
                  ...
                  onSelection(
                    Array.from(
                      {length: last - first + 1},
                      (_, offset) => order?.[first + offset] ?? first + offset,
                    ),
                    additive,
                  );
                  return;
...
function buildSites(): SiteData {
  const generator = createQtRandomGenerator(7);
...
export function DualTableLinkingExampleApp() {
  ...
  useEffect(() => {
    ...
    const {dataset, summary} = createQtDataset("100k sites", sites.records);
  ...
  return (
    <div className="qt-example-window qt-linked-window">
      <p className="qt-link-info">
      ...
      >
        ...
        />
        <section className="qt-tabs">
          <div className="qt-tab-buttons">
            <button
              type="button"
...
function buildParentsAndMetadata(): ParentData {
  const generator = createQtRandomGenerator(17);
  const random = generator.random;
...
export function MetadataOnlyLinkingExampleApp() {
  ...
  useEffect(() => {
    ...
    engineRef.current = engine;
    const {dataset, summary} = createQtDataset("100k parent geos", data.records);
    engine.loadDataset(dataset, summary);
    engine.setPointStyle({
      radius: 2.5,
      selectedRadius: 5.5,
