...
const CITIES: readonly City[] = [
  {latitude: 37.7749, longitude: -122.4194, id: "San Francisco", color: "red"},
...
export function BasicMapExampleApp() {
  ...
  const boundaryLayersRef =
    useRef<ReturnType<typeof createPackagedCountryLayers> | null>(null);
  ...
  const toggleCountries = (visible: boolean): void => {
    ...
    setBoundaryError("");
    void boundaryLayersRef.current?.setVisible(visible).catch((error: unknown) => {
          ...
          <input
            ...
            onChange={(event) => {
              ...
              if (mapTargetRef.current) {
                mapTargetRef.current.style.backgroundColor = event.target.checked
                  ? "#000000"
                  : "#ffffff";
              }
            }}
