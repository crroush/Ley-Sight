...
import {QT_USE_CASES} from "./qtUseCases";

test("parity catalog contains every numbered Qt example exactly once", () => {
  assert.equal(QT_USE_CASES.length, 22);
  assert.deepEqual(
    QT_USE_CASES.map((useCase) => useCase.id),
    Array.from({length: 22}, (_, index) => index + 1),
  );
});

test("unreleased use cases remain explicitly marked in progress", () => {
  for (const useCase of QT_USE_CASES) {
    assert.notEqual(useCase.status, "available", useCase.contract.sourcePath);
    assert.ok(useCase.remaining.length > 0);
  }
});

test("every Qt use case launches a built browser route", () => {
  ...
  for (const useCase of QT_USE_CASES) {
    ...
    assert.match(useCase.href, new RegExp(`example=${String(useCase.id).padStart(2, "0")}$`));
    assert.equal(
      useCase.contract.sourcePath,
      `examples/${String(useCase.id).padStart(2, "0")}_${
        useCase.contract.sourcePath.split(`${String(useCase.id).padStart(2, "0")}_`)[1]
      }`,
    );
