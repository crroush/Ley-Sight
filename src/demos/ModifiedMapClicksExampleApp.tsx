...
export function ModifiedMapClicksExampleApp() {
  ...
  useEffect(() => {
    ...
    map.on("singleclick", (event) => {
      ...
      const keys = clickKeysRef.current.get(original) ??
        Array.from(heldKeysRef.current);
    ...
    <main className="qt-example-window">
      <section className="qt-click-controls">
        <p>
          ...
          an orange priority target. Shift replaces the desktop Ctrl modifier because
          browsers reserve Ctrl+T for New Tab.
        </p>
        <button
          type="button"
          onClick={() => {
