...
function validateBaseLayers(
  ...
  return value.map((layer, index) => {
    ...
    rejectUnknownKeys(
      layer,
      ['id', 'name', 'type', 'url', 'attribution', 'maxZoom'],
      layerPath
    );
    const id = requiredString(layer.id, `${layerPath}.id`);
    if (ids.has(id))
      fail(`${layerPath}.id`, `duplicate id ${JSON.stringify(id)}`);
    ids.add(id);
    ...
    const url =
      layer.type === 'xyz'
        ? requiredUrl(layer.url, `${layerPath}.url`)
        : undefined;
    const attribution = optionalString(
      layer.attribution,
      `${layerPath}.attribution`
    );
    if (
      layer.maxZoom !== undefined &&
      (typeof layer.maxZoom !== 'number' ||
        !Number.isFinite(layer.maxZoom) ||
        layer.maxZoom < 0)
    ) {
...
function validateManagedLayerPresets(
  value: unknown,
  path: string
): ManagedLayerDefinition[] {
  ...
  return value.map((layer, index) => {
    ...
    if (layer.type === 'wms')
      layers = requiredString(layer.layers, `${layerPath}.layers`);
    else if (layer.layers !== undefined)
      fail(`${layerPath}.layers`, 'is only supported when type is "wms"');
    const attribution = optionalString(
      layer.attribution,
      `${layerPath}.attribution`
    );
    ...
    if (typeof layer.visible !== 'boolean')
      fail(`${layerPath}.visible`, 'must be a boolean');
    return {
      id,
      name,
      type: layer.type,
...
export function validateConfig(value: unknown): AppConfig {
  ...
  rejectUnknownKeys(
    value,
    ['csvColumnDetection', 'baseLayers', 'wmsPresets'],
    'configuration'
  );
  const csvOverrides =
    value.csvColumnDetection === undefined
      ? {}
  ...
  for (const [index, preset] of wmsPresets.entries()) {
    if (baseLayerIds.has(preset.id)) {
      fail(
        `wmsPresets[${index}].id`,
        `duplicate id ${JSON.stringify(preset.id)}`
      );
    }
  }
  return {
