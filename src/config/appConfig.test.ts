...
describe('application configuration', () => {
  it('accepts a partial configuration and preserves unspecified defaults', () => {
    ...
    assert.deepEqual(
      config.csvColumnDetection.longitude,
      DEFAULT_APP_CONFIG.csvColumnDetection.longitude
    )
  ...
  it('uses supplied base-layer and preset lists as replacements', () => {
    ...
    assert.deepEqual(
      config.baseLayers.map(({ id }) => id),
      ['local']
    )
    assert.deepEqual(
      config.wmsPresets.map(({ id }) => id),
      ['weather']
    )
  })
  ...
  it('reports the path of invalid nested definitions', () => {
    assert.throws(
      () =>
        validateConfig({
          baseLayers: [{ id: 'tiles', name: 'Tiles', type: 'xyz' }],
        }),
      /baseLayers\[0\]\.url/
    )
    assert.throws(
    ...
    assert.throws(
      () =>
        validateConfig({
          baseLayers: [{ id: 'map', name: 'Map', type: 'other' }],
        }),
      /baseLayers\[0\]\.type/
    )
    assert.throws(
      () =>
        validateConfig({
          csvColumnDetection: {
            latitude: [{ pattern: 'x', score: 1, extra: true }],
          },
        }),
  ...
  it('rejects bad regular-expression flags with their path', () => {
    assert.throws(
      () =>
        validateConfig({
          csvColumnDetection: {
            latitude: [{ pattern: 'lat', flags: 'ii', score: 1 }],
          },
        }),
