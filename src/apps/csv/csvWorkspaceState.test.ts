...
function summary(name: string, rowCount: number, timeMin: number, timeMax: number): DatasetSummary {
...
describe("csv workspace state helpers", () => {
  ...
  it("renders uniform display colors without mutating source colors", () => {
    ...
    const colors = displayColors(tab({ dataset: source, summary: summary("A", 2, 5, 6) }));
  ...
  it("persists only ready CSV tabs with complete OPFS metadata", () => {
    const ready = tab({
      ...
      persistedFiles: [{ storageId: "file-1", name: "input.csv", type: "text/csv", size: 11, lastModified: 1 }],
      timeFilterRange: [1, 2],
      timeViewRange: [0, 3],
    });
