...
test('Terrarium samples preserve negative elevations and ocean sea level', async () => {
  const negative = new TerrariumTerrainProvider(async () =>
    tileWithElevation(-430)
  );
  assert.deepEqual(
    Array.from(
      await negative.sampleGrid(new Float64Array([0]), new Float64Array([0]), 1)
    ),
    [-430]
  );
  ...
  assert.deepEqual(
    Array.from(
      await ocean.sampleGrid(new Float64Array([0]), new Float64Array([0]), 1)
    ),
    [0]
  );
});
