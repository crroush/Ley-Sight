...
const QT05_Y = [
  ...
  0.6830489532424546, 0.7447621559078171, 0.96750973243421,
  0.32582535813815194, 0.3704597060348689, 0.4695558112758079,
...
function qt14Polygon(
  ...
  const longitudeMaximum = -122.340;
  return coordinates.map(([latitude, longitude]) => [
    ((longitude - longitudeMinimum) /
      (longitudeMaximum - longitudeMinimum)) * (width - 1),
    ((latitudeMaximum - latitude) /
      (latitudeMaximum - latitudeMinimum)) * (height - 1),
  ] as const);
}
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      let value =
        ...
        0.20 * Math.sin(2.3 * fieldX) * Math.cos(1.9 * fieldY) +
        0.10 * Math.sin(7 * fieldX + 1.2 * fieldY) +
        0.08 * Math.cos(6.3 * fieldY - 1.5 * fieldX);
      value +=
        0.02 *
        Math.sin(longitude * 180) *
        Math.cos(latitude * 220);
...
async function renderRaster(
  ...
  for (let y = 0; y < request.height; y += 1) {
    ...
    for (let x = 0; x < request.width; x += 1) {
      ...
      const wave =
        0.22 *
        (Math.sin(normalizedX * 28) * Math.cos(normalizedY * 21) + 1);
