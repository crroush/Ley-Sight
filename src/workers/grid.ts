...
export function latToMercatorY(latDeg: number): number {
  const lat = Math.max(-WEB_MERCATOR_MAX_LAT_DEG, Math.min(WEB_MERCATOR_MAX_LAT_DEG, latDeg));
  return WGS84_A_M * Math.log(Math.tan(Math.PI / 4.0 + (lat * Math.PI / 180.0) / 2.0));
}
...
export function continuousExtentX(
  ...
  const estimatedTurns = Math.round((expectedWidth - baseDelta) / WEB_MERCATOR_WORLD_WIDTH_M);
  ...
  const shiftTurns = Math.round((targetX - centerX) / WEB_MERCATOR_WORLD_WIDTH_M);
...
export function planBufferedViewportRasterGrid(
  extent: { lonMin: number; latMin: number; lonMax: number; latMax: number; widthPx: number; heightPx: number; resolution: number },
  ...
  if (bufferScreenPx < 0.0) throw new Error("bufferScreenPx must be nonnegative");
  if (maximumCellsPerAxis < 1) throw new Error("maximumCellsPerAxis must be positive");

  const resolution = Math.max(1.0e-3, extent.resolution || 10000.0);
  const requestedCellM = resolution * pixelsPerCell;

  const [xMin, xMax] = continuousExtentX(extent.lonMin, extent.lonMax, resolution, extent.widthPx, referenceLonDeg);

  const latMin = Math.max(-WEB_MERCATOR_MAX_LAT_DEG, Math.min(WEB_MERCATOR_MAX_LAT_DEG, extent.latMin || -85.0));
  const latMax = Math.max(-WEB_MERCATOR_MAX_LAT_DEG, Math.min(WEB_MERCATOR_MAX_LAT_DEG, extent.latMax || 85.0));

  const yMin = Math.max(-WEB_MERCATOR_HALF_WORLD_M, latToMercatorY(Math.min(latMin, latMax)));
  const yMax = Math.min(WEB_MERCATOR_HALF_WORLD_M, latToMercatorY(Math.max(latMin, latMax)));

  const bufferM = resolution * bufferScreenPx;
  let desiredXMin = xMin - bufferM;
  let desiredXMax = xMax + bufferM;

  const isGlobalX = (desiredXMax - desiredXMin) >= 0.95 * WEB_MERCATOR_WORLD_WIDTH_M;
  if (isGlobalX) {
  ...
  const snapped = (cellM: number) => {
    ...
    if (isGlobalX) {
      const columns = Math.max(1, Math.ceil(WEB_MERCATOR_WORLD_WIDTH_M / cellM));
    ...
    const gridYMin = Math.max(-WEB_MERCATOR_HALF_WORLD_M, Math.floor(desiredYMin / cellM) * cellM);
    const gridYMax = Math.min(WEB_MERCATOR_HALF_WORLD_M, Math.ceil(desiredYMax / cellM) * cellM);
  ...
  const reduction = Math.max(1.0, initial.columns / maximumCellsPerAxis, initial.rows / maximumCellsPerAxis);
