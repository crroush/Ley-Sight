...
test('CSV mapping exposes labels and rejects duplicate coordinate columns', () => {
  ...
  assert.equal(
    screen.getByRole('button', {name: 'Load data'}).hasAttribute('disabled'),
    true
  );
});
...
test('CSV mapping resets inferred fields when another CSV replaces the open dialog', () => {
  ...
  assert.equal(
    screen.getByLabelText<HTMLSelectElement>('Time column').value,
    'time'
  );
  ...
  assert.equal(
    screen.getByLabelText<HTMLSelectElement>('Latitude column').value,
    'y'
  );
  assert.equal(
    screen.getByLabelText<HTMLSelectElement>('Longitude column').value,
    'x'
  );
  assert.equal(
    screen.getByLabelText<HTMLSelectElement>('Time column').value,
