...
export function CsvMappingDialog({
  ...
  useEffect(() => {
    ...
    Papa.parse<Record<string, string>>(files[0], {
      ...
      complete: ({data}) => {
        if (active)
          setTimestampSamples(
            data.map((row) => row[mapping.time!] ?? '').slice(0, 5)
          );
      },
    });
    return () => {
  ...
  const timestampInterpretation =
    mapping.timestampInterpretation ?? 'automatic';
  ...
  const mappingInvalid =
    !mapping.latitude || !mapping.longitude || duplicateCoordinates;
  return (
    <ModalDialog
    ...
    >
        ...
        <div className="timestamp-preview">
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setMapping((current) => ({
                  ...current,
                  timestampInterpretation: event.target
                    .value as TimestampInterpretation,
                }))
              }
            >
          ...
          <div>
            ...
            {timestampSamples.length ? (
              timestampSamples.map((sample, index) => (
                <code key={`${index}-${sample}`}>
                  <span>{sample || '(empty)'}</span>
                  <strong>
                    {formatTimestampPreview(
                      parseTimestamp(sample, timestampInterpretation)
                    )}
                  </strong>
                </code>
              ))
      ...
      <div className="form-grid">
        {fields.map(([key, label, required]) => (
          <label key={key}>
            ...
            >
              {!required && (
                <option value="">{key === 'color' ? 'Uniform' : 'None'}</option>
              )}
              {columns.map((column) => (
      ...
      {duplicateCoordinates && (
        <p
          ...
          id="csv-coordinate-mapping-error"
          role="alert"
        >
      ...
      <div className="dialog-note" id="csv-dialog-description">
        ...
        editable before loading. Longitude must be between −180° and 180° and
        latitude between −90° and 90°; invalid coordinates are skipped, never
        wrapped. Latitudes beyond Web Mercator's ±85.05112878° limit are
        retained but clamped at that limit for display and reported as
