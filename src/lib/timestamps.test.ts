...
describe('parseTimestamp', () => {
  it('parses ISO timestamps with offsets and treats zone-less values as UTC', () => {
    ...
    assert.equal(
      parseTimestamp('2024-02-29T14:34:56+02:00', 'iso'),
      1_709_210_096
    );
  ...
  it('preserves four-digit ISO years below 100', () => {
    ...
    assert.equal(
      new Date(yearZero * 1000).toISOString(),
      '0000-01-01T00:00:00.000Z'
    );
    assert.equal(
      new Date(yearNinetyNine * 1000).toISOString(),
      '0099-12-31T23:59:59.250Z'
    );
  });
  it('converts seconds, milliseconds, microseconds, nanoseconds, and Excel dates', () => {
    ...
    assert.equal(
      parseTimestamp('1700000000000', 'unix-milliseconds'),
      expected
    );
    assert.equal(
      parseTimestamp('1700000000000000', 'unix-microseconds'),
      expected
    );
    assert.equal(
      parseTimestamp('1700000000000000000', 'unix-nanoseconds'),
      expected
    );
  ...
  it('validates the supported Date boundaries', () => {
    assert.equal(
      parseTimestamp(String(MIN_TIMESTAMP_SECONDS), 'unix-seconds'),
      MIN_TIMESTAMP_SECONDS
    );
    assert.equal(
      parseTimestamp(String(MAX_TIMESTAMP_SECONDS), 'unix-seconds'),
      MAX_TIMESTAMP_SECONDS
    );
    assert.ok(
      Number.isNaN(
        parseTimestamp(String(MIN_TIMESTAMP_SECONDS - 1), 'unix-seconds')
      )
    );
    assert.ok(
      Number.isNaN(
        parseTimestamp(String(MAX_TIMESTAMP_SECONDS + 1), 'unix-seconds')
      )
    );
  });
});
