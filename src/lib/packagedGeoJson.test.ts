...
describe('packaged GeoJSON transport', () => {
  it('accepts a response already decompressed by Fetch', async () => {
    ...
    assert.equal(await decodePackagedResponse(new Response(json)), json);
  });
  ...
  it('accepts a raw gzip response', async () => {
    ...
    assert.equal(await decodePackagedResponse(new Response(bytes)), json);
  });
    ...
    const parse = (name: string): {features: unknown[]} =>
      JSON.parse(
        gunzipSync(
          readFileSync(
            new URL(
              `../../public/resources/${name}.geojson.gz`,
              import.meta.url
            )
          )
        ).toString('utf8')
