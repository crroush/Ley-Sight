...
export function reduceViewshedResult(
  ...
  if (action.type !== "START" && action.runId !== state.activeRunId) return state;
  ...
  return action.type === "SUCCESS"
    ? { ...state, status: "success", resultTimestamp: action.timestamp, errorMessage: null }
    : { ...state, status: "error", resultTimestamp: action.timestamp, errorMessage: action.message };
}
...
export function viewshedPayloadError(payload: unknown): string | null {
  ...
  if (!(value.buffer instanceof ArrayBuffer))
    return "The viewshed result did not include an image buffer.";
  if (!Number.isInteger(value.nx) || (value.nx as number) <= 0 || !Number.isInteger(value.ny) || (value.ny as number) <= 0)
    return "The viewshed result has invalid image dimensions.";
  if (!Array.isArray(value.bounds) || value.bounds.length !== 4 || !value.bounds.every((item) => typeof item === "number" && Number.isFinite(item)))
    return "The viewshed result has invalid map bounds.";
  if (value.buffer.byteLength !== (value.nx as number) * (value.ny as number) * 4)
