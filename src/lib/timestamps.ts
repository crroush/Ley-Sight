...
const NUMERIC_UNITS: Exclude<TimestampInterpretation, "automatic" | "iso">[] = [
  "unix-seconds", "unix-milliseconds", "unix-microseconds",
  "unix-nanoseconds", "excel-serial",
];

function numericSeconds(text: string, unit: typeof NUMERIC_UNITS[number]): number {
  if (!/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)$/.test(text)) return Number.NaN;
  const value = Number(text);
  if (!Number.isFinite(value)) return Number.NaN;
  switch (unit) {
    case "unix-seconds": return value;
    case "unix-milliseconds": return value / 1_000;
    case "unix-microseconds": return value / 1_000_000;
    case "unix-nanoseconds": return value / 1_000_000_000;
    case "excel-serial": return (value - 25_569) * 86_400;
  }
}
...
function isoSeconds(text: string): number {
  ...
  const [, yearText, monthText, dayText, hourText = "0", minuteText = "0", secondText = "0", fraction = "", zone = "Z"] = match;
  ...
  if (zone !== "Z") {
    ...
    const offsetHours = Number(digits.slice(0, 2)), offsetMinutes = Number(digits.slice(2));
...
export function parseTimestamp(value: unknown, interpretation: TimestampInterpretation = "automatic"): number {
  ...
  return Number.isFinite(seconds) && seconds >= MIN_TIMESTAMP_SECONDS && seconds <= MAX_TIMESTAMP_SECONDS
    ? seconds : Number.NaN;
}

export function formatTimestampPreview(seconds: number): string {
  return Number.isFinite(seconds) ? new Date(seconds * 1000).toISOString() : "Invalid timestamp";
}
