...
test("stale-run success and failure responses do not change the active result", () => {
  const running = reduceViewshedResult(initialViewshedResultState, { type: "START", runId: 2 });
  assert.equal(reduceViewshedResult(running, { type: "SUCCESS", runId: 1, timestamp: 10 }), running);
  assert.equal(reduceViewshedResult(running, { type: "ERROR", runId: 1, timestamp: 11, message: "old" }), running);
});

test("worker failure records a timestamped, user-displayable error", () => {
  const running = reduceViewshedResult(initialViewshedResultState, { type: "START", runId: 4 });
  assert.deepEqual(reduceViewshedResult(running, { type: "ERROR", runId: 4, timestamp: 99, message: "terrain unavailable" }), {
    activeRunId: 4, status: "error", resultTimestamp: 99, errorMessage: "terrain unavailable",
  });
});

test("missing and malformed image buffers are rejected", () => {
  assert.match(viewshedPayloadError({ nx: 1, ny: 1, bounds: [0, 0, 1, 1] })!, /buffer/);
